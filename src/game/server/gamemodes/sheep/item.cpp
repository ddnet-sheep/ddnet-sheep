...
bool CGameControllerSheep::ExecuteLoadItems(IDbConnection *pSqlServer, const ISqlData *pGameData, char *pError, int ErrorSize) {
	...
	if(!pSqlServer->PrepareStatement("SELECT id, type, name, description FROM sheep_items", pError, ErrorSize))
	    return false;
    ...
    do {
        ...
        CItem item;
        item.m_Variant = static_cast<EItemVariant>(pSqlServer->GetInt64(1));
        item.m_Type = static_cast<EItemType>(pSqlServer->GetInt(2));
        pSqlServer->GetString(3, item.m_Name, sizeof(item.m_Name));
        pSqlServer->GetString(4, item.m_Description, sizeof(item.m_Description));

        pResult->m_Items[item.m_Variant] = item;
    } while(!End);
...
bool CGameControllerSheep::ExecuteLoadAccountItem(IDbConnection *pSqlServer, const ISqlData *pGameData, char *pError, int ErrorSize) {
	...
	if(!pSqlServer->PrepareStatement("SELECT item_id, amount, state FROM sheep_account_item WHERE account_id = ?", pError, ErrorSize))
	    return false;
    ...
    do {
        ...
        CAccountItem item;
        item.m_Variant = static_cast<EItemVariant>(pSqlServer->GetInt(1));
        item.m_Amount = pSqlServer->GetInt64(2);
        item.m_State = pSqlServer->GetInt(3);

        pResult->m_AccountItem[item.m_Variant] = item;
    } while(!End);
