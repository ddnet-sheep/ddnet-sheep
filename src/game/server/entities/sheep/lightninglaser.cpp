...
void CLightningLaser::InitTarget() {
	...
	for(int i = 0; i < MAX_CLIENTS; i ++) {
		...
		{
			float TargetAngle = CMath::GetAngle(GameServer()->GetPlayerChar(i)->GetPos() - m_Pos) * 180 / pi + 90.f;
...
void CLightningLaser::GenerateLights()
{
	...
	for(int i = 0; i < m_Count; i ++)
	{
		...
		randShot = CMath::GetAngle(m_Dir) + randDir * pi / 180.f;
				
		m_aaPositions[i][POS_END] = m_aaPositions[i][POS_START] + CMath::GetDir(randShot) * m_Length;
		...
		for(int j = 0; j < MAX_CLIENTS; j++)
		{
			...
			if (closest_point_on_line(m_aaPositions[i][POS_END], m_aaPositions[i][POS_START], pChrs->GetPos(), Point))
			{
				...
				if(distance(Point, pChrs->GetPos()) < 10)
				{
					...
					pChrs->Freeze();
					pChrs->SetVelocity(vec2(0.f, 0.f));	
				}
			}
		}
	}
...
void CLightningLaser::Snap(int SnappingClient)
{
	...
	int Start = (int)std::max((float)ceil(Percentage * m_Count / 100.f), 1.f);
